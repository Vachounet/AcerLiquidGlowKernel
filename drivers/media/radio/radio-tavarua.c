...
static unsigned int FM_DLL =0;
#define fm_printk(level,fmt,args...) if (level <= FM_DLL) printk("FM_radio_tavarua:" fmt,##args);
...
static int is_bahama(void)
{
	...
	case BAHAMA_ID:
		fm_printk(1,"It is Bahama\n");
		return 1;

	case MARIMBA_ID:
		fm_printk(1,"It is Marimba\n");
		return 0;
	default:
				...
				unsigned char offset, int len)
{
	...
	fm_printk(1,"I2C Slave: %x, Read Offset(%x): Data [",
	...
	if (retval > 0) {
		for (i = 0; i < len; i++)
			fm_printk(1,"%02x ", radio->registers[offset+i]);
		fm_printk(1," ]\n");
			...
			unsigned char offset, unsigned char value)
{
	...
	fm_printk(1,"I2C Slave: %x, Write Offset(%x): Data[",
						radio->marimba->mod_id,
						offset);
	...
	if (retval > 0) {
		if (offset < RADIO_REGISTERS) {
			...
			fm_printk(1,"%02x ", radio->registers[offset]);
		}
		fm_printk(1," ]\n");
	}
	return retval;
	...
	if (retval > 0) { /* if write successful, update internal state too */
		for (i = 0; i < len; i++) {
			if ((offset+i) < RADIO_REGISTERS) {
				...
				fm_printk(1,"%x ",  radio->registers[offset+i]);
			}
		}
		fm_printk(1," ]\n");
	}
...
static int xfr_intf_own(struct tavarua_device *radio)
{
	...
	if (radio->xfr_in_progress) {
		...
		fm_printk(1,"gained ownership of xfr\n");
	...
	for (i = 0; i < TAVARUA_XFR_MAX; i++) {
		if (radio->pending_xfrs[i]) {
			...
			case TAVARUA_XFR_AF_LIST:
				...
				break;
			default:
				fm_printk(0,"%s: Unsupported XFR %d\n",
					 __func__, xfr);
			}
			radio->pending_xfrs[i] = 0;
			fm_printk(1,"resurrect xfr %d\n", i);
			}
	}
	return;
}
				...
				enum tavarua_evt_t event)
{
	...
	fm_printk(0,"updating event_q with event %x\n", event);
...
static void tavarua_handle_interrupts(struct tavarua_device *radio)
{
	...
	if (radio->registers[STATUS_REG1] & TUNE) {
		...
		if (radio->tune_req) {
			fm_printk(0,"Tune INT is pending\n");
			mutex_unlock(&radio->lock);
			return;
		}
	}
	...
	if (radio->registers[STATUS_REG3] & TRANSFER) {
		...
		case RDS_PS_6:
			...
			break;
		case RDS_RT_0:
			fm_printk(1,"RT Header\n");
			copy_from_xfr(radio, TAVARUA_BUF_RT_RDS, 5);
			radio->xfr_bytes_left = radio->registers[XFRCTRL+1]
									& 0x7F;
			fm_printk(1,"RT RDS Length: %d\n", radio->xfr_bytes_left);
		...
		case RDS_RT_3:
		case RDS_RT_4:
			fm_printk(1,"xfr interrupt RT data\n");
		...
		case RDS_CONFIG:
			...
			break;
		case RX_STATIONS_0:
			fm_printk(1,"Search list has %d stations\n",
			...
			if (radio->xfr_bytes_left > 14) {
				...
				fm_printk(1,"In else RX_STATIONS_0\n");
				copy_from_xfr(radio, TAVARUA_BUF_SRCH_LIST,
		...
		case RX_STATIONS_1:
			...
			break;
		case PHY_TXGAIN:
			fm_printk(1,"read PHY_TXGAIN is successful");
			complete(&radio->sync_req_done);
			break;
		case (0x80 | RX_CONFIG):
		...
		case (0x80 | RDS_RT_3):
		case (0x80 | RDS_RT_4):
			fm_printk(1,"xfr interrupt RT data Sent\n");
			complete(&radio->sync_req_done);
			break;
		/*TX Specific transfer */
		case (0x80 | RDS_PS_0):
			fm_printk(1,"PS Header Sent\n");
			complete(&radio->sync_req_done);
			break;
		case (0x80 | RDS_PS_1):
		case (0x80 | RDS_PS_2):
		...
		case (0x80 | RDS_PS_5):
		case (0x80 | RDS_PS_6):
			fm_printk(1,"xfr interrupt PS data Sent\n");
			complete(&radio->sync_req_done);
			break;
		case (0x80 | PHY_TXGAIN):
			fm_printk(1,"write PHY_TXGAIN is successful");
			complete(&radio->sync_req_done);
			break;
		default:
			fm_printk(0,"UNKNOWN XFR = %d\n", xfr_status);
		}
		if (!radio->xfr_in_progress)
			start_pending_xfr(radio);
	...
	if (radio->registers[STATUS_REG3] & ERROR) {
		...
		fm_printk(1,"retval of ERROR_CODE read : %d\n", retval);
#endif
		fm_printk(0,"ERROR STATE\n");
	}

	mutex_unlock(&radio->lock);
	fm_printk(1,"Work is done\n");
...
static void fm_shutdown(struct work_struct *work)
{
	struct tavarua_device *radio = container_of(work,
					struct tavarua_device, work.work);
	fm_printk(0,"%s: Releasing the FM I2S GPIO\n", __func__);
	if (radio->pdata->config_i2s_gpio != NULL)
		radio->pdata->config_i2s_gpio(FM_I2S_OFF);
	fm_printk(0,"%s: Shutting down FM SOC\n", __func__);
...
static int tavarua_request_irq(struct tavarua_device *radio)
{
  ...
  */
	...
	if (retval < 0) {
		fm_printk(0,"Couldn't acquire FM gpio %d\n", irq);
		return retval;
	} else {
		fm_printk(1,"FM GPIO %d registered\n", irq);
	}
	retval = enable_irq_wake(irq);
	if (retval < 0) {
		fm_printk(0,"Could not enable FM interrupt\n ");
		free_irq(irq , radio);
	}
	return retval;
}
				...
				int region)
{
	...
	*/
	fm_printk(1,"%s: The region selected from APP is"
	...
	*/
		if (adie_type_bahma) {
			fm_printk(1,"Adie type : Bahama\n");
			fm_printk(1,"%s: Enabling the 200KHz enforcer for"
	...
	*/
		if (adie_type_bahma) {
			fm_printk(1,"Adie type : Bahama\n");
			fm_printk(1,"%s: Disabling the 200KHz enforcer for"
				" Region : %d", __func__, region);
			/*
		...
		break;
	default:
		fm_printk(1,"%s: Defaulting in case of Enabling/Disabling"
	...
	case TAVARUA_REGION_OTHER:
		if (adie_type_bahma) {
			fm_printk(1,"Adie type : Bahama\n");
			fm_printk(1,"%s: Configuring the channel-spacing as 50KHz"
	...
	case TAVARUA_REGION_JAPAN_WIDE:
		...
		break;
	default:
		fm_printk(1,"%s: Defualting in case of Channel-Spacing", __func__);
		break;
	}
...
static int tavarua_search(struct tavarua_device *radio, int on, int dir)
{
	...
	fm_printk(1,"SRCHCTRL <%x>\n", radio->registers[SRCHCTRL]);
	fm_printk(1,"Search Started\n");
				...
				int req_region)
{
	...
	if (retval < 0) {
		fm_printk(0,"failed to get RADIO_CONFIG\n");
		return retval;
	}
	...
	if (retval < 0) {
		fm_printk(0,"Could not set regional settings\n");
		return retval;
	}
	...
	if (radio->enable_optimized_srch_alg) {
		fm_printk(1,"Optimized Srch Algorithm!!!");
		optimized_search_algorithm(radio, region);
	} else {
		fm_printk(1,"Native Srch Algorithm!!!");
...
static int tavarua_fops_open(struct file *file)
{
	...
	PM_LOG_EVENT(PM_LOG_ON, PM_LOG_FM_RADIO);
	...
	if (!wait_for_completion_timeout(&radio->sync_req_done,
		msecs_to_jiffies(wait_timeout))) {
		retval = -1;
		fm_printk(0,"Timeout waiting for initialization\n");
	}
...
config_i2s_err:
	...
	PM_LOG_EVENT(PM_LOG_OFF, PM_LOG_FM_RADIO);
...
static int tavarua_fops_release(struct file *file)
{
	...
	fm_printk(0,"In %s", __func__);


	fm_printk(1,"%s, Disable IRQs\n", __func__);
	/* disable irq */
...
exit:
	fm_printk(0,"%s, Calling fm_shutdown\n", __func__);
	queue_delayed_work(radio->wqueue, &radio->work,
		...
		struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_PRIVATE_TAVARUA_INTDET:
		...
		fm_printk(1,"INT_DET:Sync write success\n");
		/*Wait for the XFR interrupt */
		msleep(TAVARUA_DELAY*10);
		/* Read the XFRDAT0 register populated by FM SoC */
		retval = tavarua_read_registers(radio, XFRDAT0, 3);
		if (retval < 0) {
			fm_printk(0,"INT_DET: Read failure\n");
			return retval;
		}
	...
	case V4L2_CID_PRIVATE_TAVARUA_SIGNAL_TH:
		...
		if (retval < 0) {
			fm_printk(1,"[G IOCTL=V4L2_CID_PRIVATE_TAVARUA_SIGNAL_TH]\n");
			fm_printk(1,"sync_read_xfr error: [retval=%d]\n", retval);
			break;
		}
		/* Since RMSSI Threshold is signed value */
		cRmssiThreshold = (signed char)xfr_buf[0];
		ctrl->value  = cRmssiThreshold;
		fm_printk(1,"cRmssiThreshold: %d\n", cRmssiThreshold);
		break;
	...
	case V4L2_CID_PRIVATE_TAVARUA_RSSI_DELTA:
		retval = sync_read_xfr(radio, RX_CONFIG, xfr_buf);
		if (retval < 0) {
			fm_printk(0,"V4L2_CID_PRIVATE_TAVARUA_RSSI_DELTA]\n");
			fm_printk(0,"sync_read_xfr [retval=%d]\n", retval);
			break;
		}
	...
	case V4L2_CID_PRIVATE_INTF_LOW_THRESHOLD:
		...
		for (cnt = 0; cnt < 4; cnt++)
			fm_printk(1, "On-Channel data set is : 0x%x\t",
		...
		fm_printk(1, "The On Channel Threshold value is : 0x%x", ctrl->value);
		break;
	case V4L2_CID_PRIVATE_INTF_HIGH_THRESHOLD:
		...
		for (cnt = 0; cnt < 4; cnt++)
			fm_printk(1, "Off-channel data set is : 0x%x\t",
				(int)radio->registers[XFRDAT0+cnt]);
		...
		ctrl->value =	LSH_DATA(radio->registers[XFRDAT0],   24) |
				...
				LSH_DATA(radio->registers[XFRDAT0+2],  8) |
				(radio->registers[XFRDAT0+3]);
		fm_printk(1, "The Off Channel Threshold value is : 0x%x", ctrl->value);
		break;
	/*
			...
			struct v4l2_ext_controls *ctrl)
{
	...
	case V4L2_CID_RDS_TX_PS_NAME: {
		fm_printk(1,"In V4L2_CID_RDS_TX_PS_NAME\n");
		...
		while (bytes_left) {
			chunk_index++;
			fm_printk(1,"chunk is %d", chunk_index);
			bytes_to_copy = min(bytes_left, XFR_REG_NUM);
			/*Clear the tx_data */
			...
			if (retval < 0)	{
				fm_printk(1,"sync_write_xfr:  %d", retval);
				return retval;
			}
			bytes_copied += bytes_to_copy;
			bytes_left -= bytes_to_copy;
		...
		if (retval < 0)	{
			fm_printk(1,"sync_write_xfr returned %d", retval);
			return retval;
		}
	} break;
	case V4L2_CID_RDS_TX_RADIO_TEXT: {
		chunk_index = 0;
		bytes_copied = 0;
		fm_printk(1,"In V4L2_CID_RDS_TX_RADIO_TEXT\n");
		/*Pass a sample PS string */
		fm_printk(1,"Passed RT String : %s\n",
		...
		if (retval < 0)
			return retval;
		fm_printk(1,"done RT writing: %d\n", retval);
	} break;
	default:
	{
		fm_printk(0,"Shouldn't reach here\n");
		retval = -1;
	}
	}
	return retval;
		...
		struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_PRIVATE_TAVARUA_STATE:
		...
		if (((ctrl->value == FM_RECV) || (ctrl->value == FM_TRANS))
				    && !(radio->registers[RDCTRL] &
							ctrl->value)) {
			fm_printk(1,"clearing flags\n");
			init_completion(&radio->sync_xfr_start);
			init_completion(&radio->sync_req_done);
			radio->xfr_in_progress = 0;
			radio->xfr_bytes_left = 0;
			fm_printk(1,"turning on ..\n");
			...
			if (retval >= 0) {
				...
				retval = tavarua_write_register(radio,
					IOCTRL, value);
				if (retval < 0)
					fm_printk(1,"SMute and SBlending"
						"not enabled\n");
			}
		}
		/* check if off */
		else if ((ctrl->value == FM_OFF) && radio->registers[RDCTRL]) {
			fm_printk(1,"turning off...\n");
			...
			fm_printk(0,"%s, Disable All Interrupts\n", __func__);
			/* disable irq */
	...
	case V4L2_CID_PRIVATE_TAVARUA_SET_AUDIO_PATH:
		fm_printk(1,"Setting audio path ...\n");
		if (ctrl->value == FM_DIGITAL_PATH) {
			fm_printk(1,"Digital audio path enabled ...\n");
			retval = tavarua_set_audio_path(
				TAVARUA_AUDIO_OUT_DIGITAL_ON,
				TAVARUA_AUDIO_OUT_ANALOG_OFF);
			if (retval < 0) {
				fm_printk(0,"Error in tavarua_set_audio_path"
					" %d\n", retval);
			}
		} else if (ctrl->value == FM_ANALOG_PATH) {
			fm_printk(1,"Analog audio path enabled ...\n");
			retval = tavarua_set_audio_path(
	...
	case V4L2_CID_PRIVATE_TAVARUA_SRCH_ALGORITHM:
		...
		fm_printk(1,"V4L2_CID_PRIVATE_TAVARUA_SRCH_ALGORITHM : %d",
	...
	case V4L2_CID_PRIVATE_TAVARUA_SIGNAL_TH:
		retval = sync_read_xfr(radio, RX_CONFIG, xfr_buf);
		if (retval < 0)	{
			fm_printk(0,"V4L2_CID_PRIVATE_TAVARUA_SIGNAL_TH]\n");
			fm_printk(0,"sync_read_xfr [retval=%d]\n", retval);
			break;
		}
		...
		if (retval < 0) {
			fm_printk(0,"V4L2_CID_PRIVATE_TAVARUA_SIGNAL_TH]\n");
			fm_printk(0,"sync_write_xfr [retval=%d]\n", retval);
			break;
		}
		break;
	case V4L2_CID_PRIVATE_TAVARUA_SRCH_PTY:
	...
	case V4L2_CID_PRIVATE_TAVARUA_LP_MODE:
		...
		if (ctrl->value == radio->lp_mode)
			break;
		if (ctrl->value) {
			fm_printk(1,"going into low power mode\n");
			retval = tavarua_disable_interrupts(radio);
		} else {
			fm_printk(1,"going into normal power mode\n");
	...
	case V4L2_CID_PRIVATE_INTF_LOW_THRESHOLD:
		...
		for (cnt = 3; cnt < 7; cnt++) {
			fm_printk(1,"On-channel data to be poked is : %d",
				(int)xfr_buf[cnt]);
		}
	...
	case V4L2_CID_PRIVATE_INTF_HIGH_THRESHOLD:
		...
		for (cnt = 3; cnt < 7; cnt++) {
			fm_printk(1,"Off-channel data to be poked is : %d",
				(int)xfr_buf[cnt]);
		}
	...
	case V4L2_CID_PRIVATE_TAVARUA_STOP_RDS_TX_PS_NAME: {
			fm_printk(1,"In STOP_RDS_TX_PS_NAME\n");
			/*Pass a sample PS string */
			memset(tx_data, '0', XFR_REG_NUM);
			fm_printk(1,"Writing PS header\n");
			retval = sync_write_xfr(radio, RDS_PS_0, tx_data);
			fm_printk(1,"retval of PS Header write: %d", retval);
	...
	case V4L2_CID_PRIVATE_TAVARUA_STOP_RDS_TX_RT: {
			...
			fm_printk(1,"Writing RT header\n");
			retval = sync_write_xfr(radio, RDS_RT_0, tx_data);
			fm_printk(1,"retval of Header write: %d", retval);
	...
	case V4L2_CID_TUNE_POWER_LEVEL: {
		...
		fm_printk(1,"return for PHY_TXGAIN is %d", retval);
		if (retval < 0) {
			fm_printk(0,"read failed");
			break;
		}
		xfr_buf[2] = tx_power_lvl_config[ctrl->value];
		retval = sync_write_xfr(radio, PHY_TXGAIN, xfr_buf);
		fm_printk(1,"return for write PHY_TXGAIN is %d", retval);
		if (retval < 0)
			fm_printk(0,"write failed");
	} break;
	/*These IOCTL's are place holders to keep the
	driver compatible with change in frame works for IRIS */
					...
					struct v4l2_frequency *freq)
{
	...
	fm_printk(1,"Calling tavarua_set_freq\n");
	...
	if (retval < 0) {
		...
		if (!wait_for_completion_timeout(&radio->sync_req_done,
			msecs_to_jiffies(wait_timeout))) {
			fm_printk(0,"Timeout: No Tune response");
				...
				struct v4l2_buffer *buffer)
{
	...
	fm_printk(0,"%s: requesting buffer %d\n", __func__, buf_type);
	...
	if ((buf_type < TAVARUA_BUF_MAX) && (buf_type >= 0)) {
		...
		fm_printk(0,"invalid buffer type\n");
		return -EINVAL;
	}
					...
					enum radio_state_t state)
{
    ...
    if (radio->chipID == MARIMBA_2_1 || radio->chipID == BAHAMA_1_0
		...
		fm_printk(1,"Setting interrupts\n");
...
int tavarua_set_audio_path(int digital_on, int analog_on)
{
	...
	if (!radio)
		return -ENOMEM;
	/* RX */
	fm_printk(0,"%s: digital: %d analog: %d\n", __func__, digital_on, analog_on);
	...
	*/
	...
	if (!radio->pdata->is_fm_soc_i2s_master) {
		fm_printk(1,"FM SoC is I2S Slave\n");
		SET_REG_FIELD(radio->registers[AUDIOCTRL],
		(0),
		I2SCTRL_OFFSET,
		I2SCTRL_MASK);
	} else {
		fm_printk(1,"FM SoC is I2S Master\n");
		SET_REG_FIELD(radio->registers[AUDIOCTRL],
		(1),
		I2SCTRL_OFFSET,
		I2SCTRL_MASK);
...
static int  __init tavarua_probe(struct platform_device *pdev)
{
	...
	{
		debugfs_create_u32("fm_dll", S_IRUGO | S_IWUGO,
			kernel_debuglevel_dir, (u32 *)(&FM_DLL));
	}
	else
	{
		printk(KERN_ERR "kernel_debuglevel_dir fm_dll Fail\n");
	}
