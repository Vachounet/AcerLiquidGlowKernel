...
int android_get_battery_info(void)
{
	int percentage = 0xff;
	if (_android_dev && _android_dev->cdev && _android_dev->cdev->gadget)
	{
		percentage = msm72k_get_battery_Info();
	}
	return percentage;
}

int android_enable_charging(int enable)
{
        int ret = 0;
        if (_android_dev && _android_dev->cdev && _android_dev->cdev->gadget) {
                ret = msm72k_enable_charging(_android_dev->cdev->gadget,enable);
        }
        return ret;
}
...
static DEVICE_ATTR(remote_wakeup, S_IRUGO,
		remote_wakeup_show, NULL);
