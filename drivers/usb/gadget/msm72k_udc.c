...
{
	unsigned long flags;
	char *usb_state[]={"USB_STATE_NOTATTACHED","USB_STATE_POWERED","USB_STATE_UNAUTHENTICATED","USB_STATE_RECONNECTING","USB_STATE_DEFAULT","USB_STATE_ADDRESS","USB_STATE_CONFIGURED","USB_STATE_SUSPENDED"};
...
static inline enum chg_type usb_get_chg_type(struct usb_info *ui)
{
	...
	{
		if ((readl(USB_PORTSC) & PORTSC_LS) == PORTSC_LS_DM)
		{
			return USB_CHG_TYPE__OTHERWALLCHARGER;
		}
		else
		{
...
#define USB_OTHERWALLCHARGER_CHG_CURRENT 500 
static int usb_get_max_power(struct usb_info *ui)
{
	...
	if (temp == USB_CHG_TYPE__OTHERWALLCHARGER)
		return USB_OTHERWALLCHARGER_CHG_CURRENT;
...
int msm72k_get_battery_Info()
{
	int percentage = 0xff;
	percentage = msm_otg_get_battery_Info();
	return percentage;
}
int msm72k_enable_charging(struct usb_gadget *_gadget,int enable)
{
	struct usb_info *ui = container_of(_gadget,struct usb_info,gadget);
	struct msm_otg *otg = to_msm_otg(ui->xceiv);
	if (enable)
	{
		atomic_set(&otg->chg_type,USB_CHG_TYPE__FASTERCHARGER);
		otg_set_power(ui->xceiv,1000);
	}
	else
	{
