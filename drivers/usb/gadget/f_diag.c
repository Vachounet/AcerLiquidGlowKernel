...
extern int android_get_battery_info(void);
extern int android_enable_charging(int enable);
...
int usb_diag_enable_charging(int enable)
{
	int result =0;
	
	result = android_enable_charging(enable);
	return result;
}
EXPORT_SYMBOL(usb_diag_enable_charging);

int usb_diag_read_battery_Info()
{
	int capacity =0;
	
	capacity = android_get_battery_info();
	return capacity;
}
EXPORT_SYMBOL(usb_diag_read_battery_Info);
