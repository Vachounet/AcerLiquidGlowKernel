...
atomic_t update_chg_type;
atomic_t update_vbus_draw;
DEFINE_SEMAPHORE(g_update_chg_status);
...
#ifdef CONFIG_EXTERNAL_CHARGER
static struct msm_otg_chg_notify *msm_otg_chg = NULL;

void msm_otg_chg_connected_register(struct msm_otg_chg_notify *otg_chg_connected)
{
	printk("--%s--\n", __func__);

	msm_otg_chg = otg_chg_connected;
}
EXPORT_SYMBOL(msm_otg_chg_connected_register);

void msm_otg_chg_connected_unregister(struct msm_otg_chg_notify *otg_chg_connected)
{
	printk("--%s--\n", __func__);

	msm_otg_chg = NULL;
}
EXPORT_SYMBOL(msm_otg_chg_connected_unregister);

static void msm_otg_resent_connected_work(struct work_struct *work)
{
	struct msm_otg *dev;

	
	dev = container_of(work, struct msm_otg, resent_connected_work.work);

	if (msm_otg_chg && msm_otg_chg->msm7x27a_chg_connected && atomic_read(&update_chg_type)) {
		msm_otg_chg->msm7x27a_chg_connected(dev->otg_chg_type);
		atomic_set(&update_chg_type,0);
		
	} else {
		atomic_set(&update_chg_type,1);
		
		schedule_delayed_work(&dev->resent_connected_work, 100); 
	}
	
}

static void msm_otg_resent_vbus_draw(struct work_struct *work)
{
	struct msm_otg *dev;

	dev = container_of(work, struct msm_otg, resent_vbus_draw_work.work);

	if (msm_otg_chg && msm_otg_chg->msm7x27a_chg_vbus_draw && atomic_read(&update_vbus_draw)) {
		atomic_set(&update_vbus_draw,0);
		msm_otg_chg->msm7x27a_chg_vbus_draw(dev->otg_chg_ma);
	} else {
		atomic_set(&update_vbus_draw,1);
		
		schedule_delayed_work(&dev->resent_vbus_draw_work, 100); 
	}
}
#endif
...
int msm_otg_get_battery_Info(void)
{
	int voltage =0;
#ifdef CONFIG_EXTERNAL_CHARGER
	if (msm_otg_chg && msm_otg_chg->msm7x27a_chg_connected) {
		voltage = msm_otg_chg->msm7x27a_get_batt_capacity();
	}
#endif
	return voltage;
}
...
static int msm_otg_set_power(struct otg_transceiver *xceiv, unsigned mA)
{
	...
	unsigned 		charge = mA;
	static int		curr_charge = -1;
	int			change_chg_type =0;


	
	/* Call chg_connected only if the charger has changed */
	down(&g_update_chg_status);
	if (new_chg ==  USB_CHG_TYPE__WALLCHARGER || new_chg == USB_CHG_TYPE__OTHERWALLCHARGER )
	{
		if ((dev->otg.gadget->speed != USB_SPEED_UNKNOWN))
		{
			if (androidboot_mode != ANDROIDBOOTMODE_BOOTOFFCHARGE)
			{
				atomic_set(&dev->chg_type,USB_CHG_TYPE__SDP);
				new_chg =USB_CHG_TYPE__SDP;
				charge = 500;
				atomic_set(&update_chg_type,0);
			}
			else
			{
				if (dev->otg.gadget->speed == USB_SPEED_HIGH)
				{
                               	atomic_set(&dev->chg_type,USB_CHG_TYPE__SDP);
                                	new_chg =USB_CHG_TYPE__SDP;
                                	charge = 500;
		...
		if (!atomic_read(&dev->chg_det) && dev->otg.state == OTG_STATE_B_IDLE ) return 0;

	}
	if ((new_chg != curr_chg && pdata->chg_connected) || atomic_read(&update_chg_type)) {
		curr_chg = new_chg;
		change_chg_type = 1;
		...
		if (msm_otg_chg && msm_otg_chg->msm7x27a_chg_connected) {
			...
			msm_otg_chg->msm7x27a_chg_connected(new_chg);
		} else {
			atomic_set(&update_chg_type,1);
			dev->otg_chg_type = new_chg;
			schedule_delayed_work(&dev->resent_connected_work, 100); 
		}
	#endif
	...
	if (((charge || (new_chg != USB_CHG_TYPE__WALLCHARGER || new_chg != USB_CHG_TYPE__OTHERWALLCHARGER)) && (curr_charge != charge || change_chg_type)) || atomic_read(&update_vbus_draw)) {
		...
		curr_charge = charge;
		if (msm_otg_chg && msm_otg_chg->msm7x27a_chg_vbus_draw) {
			atomic_set(&update_vbus_draw,0);
			msm_otg_chg->msm7x27a_chg_vbus_draw(charge);
		} else {
			dev->otg_chg_ma = charge;
			atomic_set(&update_vbus_draw,1);
			schedule_delayed_work(&dev->resent_vbus_draw_work, 100); 
		}
	#endif
...
#ifdef CONFIG_EXTERNAL_CHARGER
	if (!msm_otg_chg) {
		INIT_DELAYED_WORK(&dev->resent_connected_work, msm_otg_resent_connected_work);
		INIT_DELAYED_WORK(&dev->resent_vbus_draw_work, msm_otg_resent_vbus_draw);
	}
#endif
