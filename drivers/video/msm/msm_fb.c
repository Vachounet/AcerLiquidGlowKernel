...
#define OFFCHARGING_IMAGE_FILE "/offcharginglogo.rle"
...
static int get_vchg_voltage(int *volt)
{
	...
	arg1 = SMEM_PC_OEM_AC_WALL_READ_VOLTAGE;
	ret = msm_proc_comm(PCOM_CUSTOMER_CMD1, &arg1, &arg2);
	if(ret != 0) {
		pr_err("%s, msm_proc_comm ac volt result:%d\n", __func__, ret);
		*volt = 0;
	} else { 
		*volt = arg2;
		MSM_FB_DEBUG("--%s: ac_volt volt(%d)--\n", __func__, arg2);
	}

	arg1 = SMEM_PC_OEM_USB_WALL_READ_VOLTAGE;
	ret = msm_proc_comm(PCOM_CUSTOMER_CMD1, &arg1, &arg2);
	if(ret != 0) {
		pr_err("%s, msm_proc_comm usb volt result:%d\n", __func__, ret);
	
	} else { 
	
		MSM_FB_DEBUG("--%s: usb_volt volt(%d)--\n", __func__, arg2);
	}

	return ret; 
}

static int get_pwrkey_state(int *state)
{
	int arg1 = SMEM_PC_OEM_PWRKEY_STATUS, arg2 = 0, ret;
...
static int set_reboot_vibrator(void)
{
	int arg1 = SMEM_PC_OEM_VIBRATOR_CTRL, arg2 = PMIC_VIBRATOR_LEVEL, ret;

	ret = msm_proc_comm(PCOM_CUSTOMER_CMD1, &arg1, &arg2);
	if(ret != 0) {
		pr_err("%s, msm_proc_comm result:%d\n", __func__, ret);
	} else { 
		MSM_FB_DEBUG("--%s, vibrating to reboot--\n", __func__);
	}
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	#ifdef POWER_OFF_CHARING_POLLING
		do {
			if (!get_vchg_voltage(&temp))
				i++;
		} while (i < 5);

		if (get_vchg_voltage(&temp) < 0) {
			printk(KERN_ERR "%s, read vchg voltage failed\n", __func__);
		} else {
			if (temp < 2000) {
				printk(KERN_ERR "%s: usb remove, shutdown!\n", __func__);
				kernel_power_off("offcharging usb removed");
			} else {
				i = 0;
				do {
				if (!get_pwrkey_state(&temp))
					i++;
				} while (i < 3);

				if (get_pwrkey_state(&temp) < 0) {
					printk(KERN_ERR "%s, read vchg voltage failed\n", __func__);
				} else {
					if (temp == 1) {
						printk(KERN_ERR "%s: user press power key, reboot!!\n", __func__);
						set_reboot_vibrator();
						kernel_restart(REBOOT_PWRKEY_MODE_STR);
					} else {
	#endif

		if (!load_565rle_image(OFFCHARGING_IMAGE_FILE)) ;	
