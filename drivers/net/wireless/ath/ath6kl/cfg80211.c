...
#define DEFAULT_BG_SCAN_PERIOD 30
			    ...
			    struct cfg80211_wowlan *wow)
{
	...
	ath6kl_dbg(ATH6KL_DBG_SUSPEND, "+%s state:%d+\n", __func__, ar->state);

	switch (mode) {
	case ATH6KL_CFG_SUSPEND_WOW:

		ath6kl_dbg(ATH6KL_DBG_SUSPEND, "wow mode suspend, state:%d\n", ar->state);
