...
#include "hif-ops.h"
#include "pm.h"
unsigned int debug_mask = ATH6KL_DBG_WLAN_CFG |
						  ATH6KL_DBG_SUSPEND |
						  ATH6KL_DBG_BOOT |
						  ATH6KL_DBG_TRC;
...
#define AR6003_BDATA_LEN	SZ_4K
#define AR6003_OTP_LEN		SZ_4K
#define AR6003_FW_LEN		SZ_128K
#define AR6003_PATCH_LEN	SZ_1K

extern void *addr_ar6003_bdata;
extern void *addr_ar6003_otp;
extern void *addr_ar6003_fw;
extern void *addr_ar6003_patch;
#endif

extern unsigned char q_wlan_flag;

void *AR6003_os_open_image(const char *filename);
void AR6003_os_close_image(void *image);
int AR6003_os_get_image_block(char *buf, int len, void *image);
...
#define WLAN_CONFIG_KEEP_ALIVE_INTERVAL 30
...
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM
void ath6kl_static_memory_init(struct ath6kl *ar)
{
	ar->fw_board = addr_ar6003_bdata;
	ar->fw_otp = addr_ar6003_otp;
	ar->fw = addr_ar6003_fw;
	ar->fw_patch = addr_ar6003_patch;

	ath6kl_dbg(ATH6KL_DBG_BOOT, "bdata=0x%p\notp=0x%p\nfw=0x%p\npatch=0x%p\n",
            ar->fw_board,
            ar->fw_otp,
            ar->fw,
            ar->fw_patch); 
...
void ath6kl_static_memory_deinit(struct ath6kl *ar)
{
	ar->fw_board = NULL;
	ar->fw_otp = NULL;
	ar->fw = NULL;
...
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM
	ath6kl_static_memory_deinit(ar);
			 ...
			 u8 **fw, size_t *fw_len)
{
	...
	if (*fw == NULL) {
		ath6kl_dbg(ATH6KL_DBG_TRC, "%s fw point is null\n", filename);
	} else {
		ath6kl_dbg(ATH6KL_DBG_TRC, "%s: using addr 0x%p\n", filename, *fw);	
	}	
...
#define AR6003_HW_FA_BOARD_DATA_FILE 	"/data/fa_boarddata.bin"     
static int ath6kl_fetch_board_file(struct ath6kl *ar)
{
	const char *filename;
	const int FA_DATA_SIZE = 16348;
	int ret,i;
	...
	memptr = kmalloc(FA_DATA_SIZE,GFP_KERNEL);
	memset(memptr,0,FA_DATA_SIZE);

	if (ar->fw_board != NULL)
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM
		memset(ar->fw_board, 0, AR6003_BDATA_LEN);
#else
		return 0;
#endif
	...
	filename = AR6003_HW_FA_BOARD_DATA_FILE;
 	image = AR6003_os_open_image(filename);
	
 	if (image == NULL)
	{
		ath6kl_dbg(ATH6KL_DBG_TRC,"FA board data file is null\n");
		goto normal;
	}

	len = AR6003_os_get_image_block((char*)memptr, FA_DATA_SIZE, image);
	ath6kl_dbg(ATH6KL_DBG_TRC,"sharmy len is %d\n",len);
	
	if((len == FA_DATA_SIZE)||(len == 0)||(len < 1024))
	{
		ath6kl_dbg(ATH6KL_DBG_TRC,"FA board data file is null or unreasonnable\n");
		goto normal;
	}
		
	memcpy(&ptr_crc,memptr + 4,2);
	ath6kl_dbg(ATH6KL_DBG_TRC,"mem crc is %04x\n", ptr_crc);
	memset(memptr + 4,0,2);
	checksum = 0;
	eeprom_ptr = (u16 *)memptr;
	for (i = 0; i < len; i += 2) 
	{
    	checksum = checksum ^ (*eeprom_ptr);
    	eeprom_ptr++;
	}
	ath6kl_dbg(ATH6KL_DBG_TRC,"checksum:0x%04x\n", checksum);
		
	if (ptr_crc != checksum) 
	{
    	ath6kl_dbg(ATH6KL_DBG_TRC,"[fa]: WIFI board data checksum fail\n");
    	goto normal;
	}

	memcpy(memptr + 4,&ptr_crc,2);
	memcpy(ar->fw_board,memptr,len);
	ar->fw_board_len = (size_t)len;
				
	kfree(memptr);
	if (image)
		AR6003_os_close_image(image);
		
	if(ar->fw_board!= NULL)
	{
		ath6kl_dbg(ATH6KL_DBG_TRC,"sharmy fw_board is %s\n",filename);			
		return 0;
	}	
			
normal:
	filename = ar->hw.fw_board;
	ath6kl_dbg(ATH6KL_DBG_TRC,"fw_board is :%s\n",filename);
	...
	while (len > sizeof(struct ath6kl_fw_ie)) {
		...
		case ATH6KL_FW_IE_OTP_IMAGE:
			ath6kl_dbg(ATH6KL_DBG_BOOT, "found otp image ie (%zd B)\n",
				ie_len);
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM
			memset(ar->fw_otp, 0, AR6003_OTP_LEN);
		...
		case ATH6KL_FW_IE_PATCH_IMAGE:
			ath6kl_dbg(ATH6KL_DBG_BOOT, "found patch image ie (%zd B)\n",
				ie_len);
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM
			memset(ar->fw_patch, 0, AR6003_PATCH_LEN);
...
static int ath6kl_upload_board_file(struct ath6kl *ar)
{
	...
	} else if (ar->version.target_ver == AR6003_HW_2_1_1_VERSION) {
		filename = AR6003_HW_2_1_1_BOARD_DATA_FILE;
	} else {
		ath6kl_err("Unknown firmware revision: %d\n", ar->version.target_ver);
		return -EINVAL; 
	}

	ath6kl_info("filename: %s\n", filename);
	...
	ret = ath6kl_bmi_write(ar, board_address,
				       ar->fw_board,
				       ar->fw_board_len);
...
#if 0
		ath6kl_dbg(ATH6KL_DBG_TRC, "Ref Clock: 19200000");
			ath6kl_bmi_reg_write(ar, 0x540678, 19200000);
#else
				ath6kl_dbg(ATH6KL_DBG_TRC, "Ref Clock: %d", ar->hw.refclk_hz);
					ath6kl_bmi_reg_write(ar, 0x540678, ar->hw.refclk_hz);
#endif
...
void *
AR6003_os_open_image(const char *filename)
{
 	struct file *fp = NULL;
...
void
AR6003_os_close_image(void *image)
{
	if (image)
...
int
AR6003_os_get_image_block(char *buf, int len, void *image)
{
