...
#define ATH6KL_ERR  0
#define ATH6KL_WARNING 1
#define ATH6KL_INFO 1
#define ATH6KL_DEBUG 1

extern unsigned int debug_mask;
extern __printf(2, 3)
int ath6kl_printk(const unsigned int level, const char *fmt, ...);

#define ath6kl_info(fmt, ...)				\
	ath6kl_printk(ATH6KL_INFO, fmt, ##__VA_ARGS__)
#define ath6kl_err(fmt, ...)					\
	ath6kl_printk(ATH6KL_ERR, fmt, ##__VA_ARGS__)
#define ath6kl_warn(fmt, ...)					\
	ath6kl_printk(ATH6KL_WARNING, fmt, ##__VA_ARGS__)
...
#define ath6kl_dbg(mask, fmt, ...)					\
	({								\
	 int rtn;							\
	 if (debug_mask & mask)						\
		rtn = ath6kl_printk(ATH6KL_DEBUG, fmt, ##__VA_ARGS__);	\
	 else								\
		rtn = 0;						\
									\
	 rtn;								\
