...
void ath6kl_tm_crash_event(struct ath6kl *ar, u8 type)
{
	...
	skb = cfg80211_testmode_alloc_event_skb(ar->wiphy, len, GFP_KERNEL);
	if (!skb) {
		ath6kl_warn("failed to allocate testmode rx skb!\n");
		return;
	}
	buf[0] = type;
	NLA_PUT_U32(skb, ATH6KL_TM_ATTR_CMD, ATH6KL_TM_CMD_CRASH);
	NLA_PUT(skb, ATH6KL_TM_ATTR_DATA, len, buf);
	ath6kl_err("trigger crash event\n");
	cfg80211_testmode_event(skb, GFP_KERNEL);
	return;
