...
#ifdef CONFIG_PANIC_LASTLOG_HOTKEY
...
static enum hrtimer_restart gpio_keypad_timer_func(struct hrtimer *timer)
{
	...
	#ifdef CONFIG_PANIC_LASTLOG_HOTKEY
	const unsigned long match_keys[] = {1, 3, 1, 3, 1, 3, 1};
	static unsigned int matched_index = 0;
	#endif
		...
		if (kp->key_state_changed) {
			if (kp->keys_pressed[0] == match_keys[matched_index])
				matched_index ++;
			else
				matched_index = 0;
			if (matched_index == ARRAY_SIZE(match_keys)) {
				matched_index = 0;
				panic(ABNORMAL_HOTKEY_REBOOT_STR);
			}
		}
		#endif
	...
	if (func == GPIO_EVENT_FUNC_INIT) {
		...
		*data = kp = kzalloc(sizeof(*kp) + sizeof(kp->keys_pressed[0]) *
		...
		for (i = 0; i < mi->ninputs; i++) {
			...
			printk(KERN_ERR "%s: gpio_direction_input for input %d\n", __func__, mi->input_gpios[i]);
