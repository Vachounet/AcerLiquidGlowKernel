...
static void qup_i2c_recover_bus_busy(struct qup_i2c_dev *dev)
{
	...
	dev_warn(dev->dev, "Bus busy cleared after %d clock cycles\n", i);
...
#ifdef CONFIG_I2C_TRANSACTION_MUTEX
extern struct mutex i2c_transaction_mutex_camera;  
extern struct mutex i2c_transaction_mutex_others;  
#endif 
...
qup_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	#ifdef CONFIG_I2C_TRANSACTION_MUTEX	
	if (adap->nr==0) {
		mutex_lock(&i2c_transaction_mutex_camera);
	}
	if (adap->nr==1) {    
		mutex_lock(&i2c_transaction_mutex_others);
	}
	#endif     
	...
	if (dev->suspended) {
		...
		#ifdef CONFIG_I2C_TRANSACTION_MUTEX		    
		if (adap->nr==0) {
			mutex_unlock(&i2c_transaction_mutex_camera);
		}	    
		if (adap->nr==1) {
			mutex_unlock(&i2c_transaction_mutex_others);
		}	
		#endif                     
...
qup_i2c_shutdown(struct platform_device *pdev)
{
	struct qup_i2c_dev	*dev = platform_get_drvdata(pdev);
...
static struct platform_driver qup_i2c_driver = {
	...
	.shutdown	= __devexit_p(qup_i2c_shutdown),
...
qup_i2c_init_driver(void)
{
	...
	#ifdef CONFIG_I2C_TRANSACTION_MUTEX	    
	mutex_init(&i2c_transaction_mutex_camera);
	mutex_init(&i2c_transaction_mutex_others);
	#endif     
