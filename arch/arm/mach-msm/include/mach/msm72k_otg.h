...
#define is_wallcharger()      ((readl(USB_PORTSC) & PORTSC_LS) == PORTSC_LS) 
...
struct msm_otg {
	...
	struct delayed_work resent_connected_work;
	struct delayed_work resent_vbus_draw_work;
...
extern int msm_otg_get_battery_Info(void); 
extern int get_usb_attached(void);
#endif
