...
static unsigned headset_gpio_config[] = {
...
static void remove_headset(void)
{
	...
	if (HSD_HEADSET == g_headset_type)
	{
		...
		if ( HSD_IRQ_ENABLE == headset_data.hook_irq_status)
		{
			...
			irq_set_irq_wake(headset_data.hook_irq, 0);
			local_irq_restore(irq_flags);
		}
	}
	g_headset_type = HSD_NONE;
	
	rc = irq_set_irq_type(headset_data.jack_irq, IRQF_TRIGGER_LOW);
	if (rc)
...
static void insert_headset(void)
{
	...
	pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_TCXO);
	HEADSET_INFO("Turn on mic en");

	{
	enum spkr_gain gain;
	pmic_spkr_get_gain(LEFT_SPKR, &gain);
	printk("%s, %d\n", __func__, gain); 
	pmic_spkr_get_gain(RIGHT_SPKR, &gain);
	printk("%s, %d\n", __func__, gain); 
	}
	...
	else
	{
		...
		rc = irq_set_irq_type(headset_data.hook_irq, IRQF_TRIGGER_HIGH);
		if (rc)
		...
		if(HSD_IRQ_DISABLE == headset_data.hook_irq_status)
		{
			...
			irq_set_irq_wake(headset_data.hook_irq, 1);
			local_irq_restore(irq_flags);
		}
	}
	...
	rc = irq_set_irq_type(headset_data.jack_irq, IRQF_TRIGGER_HIGH);
	if (rc)
...
void check_hook_key_state_work_func(struct work_struct *work)
{
	...
	if ( 1 == hook_key_status )
	{
		...
		rc = irq_set_irq_type(headset_data.hook_irq, IRQF_TRIGGER_LOW);
		if (rc)
...
static int __init_or_module headset_probe(struct platform_device *pdev)
{
	...
	ret = gpio_tlmm_config(
		headset_gpio_config[0],
		GPIO_CFG_ENABLE
		);
	if (ret < 0)
	   	printk("%s, failed to set headset_gpio_config\n", __func__);

	ret = gpio_tlmm_config(
		headset_gpio_config[1],
		GPIO_CFG_ENABLE
		);
	if (ret < 0)
	   	printk("%s, failed to set headset_gpio_config\n", __func__);
	...
	printk("%s, hook_gpio %d\n", __func__, headset_data.hook_gpio);
	...
	printk("%s, get jack_gpio %d\n", __func__, gpio_get_value(headset_data.jack_gpio));

	ret = irq_set_irq_wake(headset_data.jack_irq, 1);
	if (ret < 0)
	...
	headset_data.input->name = "c7_handset";

	input_set_capability(headset_data.input, EV_KEY, KEY_MEDIA);
	input_set_capability(headset_data.input, EV_KEY, KEY_END);
