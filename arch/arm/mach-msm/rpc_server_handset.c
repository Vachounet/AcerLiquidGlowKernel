...
#define POWER_OFF_CHARING_POLLING	1
...
static struct dentry *handset_pwrkey_dent;
struct input_dev *dummy_pwr;
static int handset_pwrkey_get(void *data, u64 *value)
{
	input_report_key(dummy_pwr, KEY_POWER, 1);
	input_sync(dummy_pwr);
	msleep(1000);
	input_report_key(dummy_pwr, KEY_POWER, 0);
	input_sync(dummy_pwr);
	*value = 1;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(handset_pwrkey_fops, handset_pwrkey_get, NULL, "%llu\n");
#endif

#ifdef POWER_OFF_CHARING_POLLING
...
#ifdef POWER_OFF_CHARING_POLLING
static int get_pwrkey_state(struct device *dev, struct device_attribute *attr, char *buf)
{

	int arg1, arg2 = 0, ret_result;

	arg1 = SMEM_PC_OEM_PWRKEY_STATUS;
	ret_result = msm_proc_comm(PCOM_CUSTOMER_CMD1, &arg1, &arg2);

	if (ret_result != 0) {
		printk(KERN_ERR "%s, msm_proc_comm result:%d\n", __func__, ret_result);
	} else {
		if(arg2 == 1)
			ret_result = 1;
		else
		...
		printk("%s, pwrkey state:%d\n", __func__, ret_result);
	}

	return sprintf(buf, "%d\n", ret_result);
}

static DEVICE_ATTR(pwrkey_state, S_IRUGO , get_pwrkey_state, NULL);

#define PMIC_VIBRATOR_LEVEL	(3100)
static int set_reboot_vibrator(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{

	unsigned int value = simple_strtoul(buf, NULL, 0);
	int arg1, arg2 = PMIC_VIBRATOR_LEVEL, ret;

	if (value == 1) {
		arg1 = SMEM_PC_OEM_VIBRATOR_CTRL;
		ret = msm_proc_comm(PCOM_CUSTOMER_CMD1, &arg1, &arg2);

		if (ret != 0) {
			printk(KERN_ERR "%s, msm_proc_comm result:%d\n", __func__, ret);
		} else {
			printk("%s, vibrating to reboot\n", __func__);
		}
	} else {
		printk(KERN_ERR "%s, unsupprot command(%d)\n", __func__, value);
	}

    return count;
}
static DEVICE_ATTR(reboot_vibrator, S_IWUSR | S_IWGRP, NULL, set_reboot_vibrator);
...
#ifdef POWER_OFF_CHARING_POLLING
	hs->dev = &pdev->dev;
	rc = sysfs_create_file(&hs->dev->kobj, &dev_attr_pwrkey_state.attr);
	if (rc)
		dev_err(hs->dev, "Failed to create pwrkey_state in sysfs\n");

	rc = sysfs_create_file(&hs->dev->kobj, &dev_attr_reboot_vibrator.attr);
	if (rc)
		dev_err(hs->dev, "Failed to create reboot_vibrator in sysfs\n");
#endif
...
#ifdef CONFIG_DEBUG_FS
	handset_pwrkey_dent = debugfs_create_dir(ipdev->name, NULL);
	if (handset_pwrkey_dent) {
		debugfs_create_file("pwrkey", 0444, handset_pwrkey_dent, NULL, &handset_pwrkey_fops);
	}
#endif
