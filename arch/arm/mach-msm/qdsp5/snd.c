...
#define ONCRPC_SND_EC_CTL_PROC 26 
...
typedef enum {
  SND_EC_NS_FORCE_DISABLED,     
  SND_EC_NS_NORMAL_OPERATION,    
...
struct snd_ec_ctl_msg {
	struct rpc_request_hdr hdr;
	struct rpc_snd_ec_ctl_args args;
};
...
static long snd_ec_enable(unsigned long arg)
{
	struct snd_ctxt *snd = &the_snd;
	struct snd_ec_ctl_msg ec_msg;
	int rc = 0;

	if ((arg != 1) && (arg != 0))
		return -EINVAL;

	ec_msg.args.ec_ctl = cpu_to_be32(arg);

	ec_msg.args.cb_func = -1;
	ec_msg.args.client_data = 0;

	MM_DBG("snd_ec_ctl %ld,%d\n", arg, ec_msg.args.ec_ctl);

	rc = msm_rpc_call(snd->ept,
		ONCRPC_SND_EC_CTL_PROC,
		&ec_msg, sizeof(ec_msg), 5 * HZ);
	return rc;
}
...
static ssize_t snd_ec_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t status;
	struct snd_ctxt *snd = &the_snd;
	...
	if (sysfs_streq(buf, "enable"))
		status = snd_ec_enable(SND_EC_NS_NORMAL_OPERATION);
	else if (sysfs_streq(buf, "disable"))
		status = snd_ec_enable(SND_EC_NS_FORCE_DISABLED);
	else
		status = -EINVAL;
...
static DEVICE_ATTR(ec, S_IWUSR | S_IRUGO,
		NULL, snd_ec_store);
...
static ssize_t debug_write_proc(struct file *file, const char __user *buf,
				 size_t count, loff_t *ppos)
{
	...
	if (temp == 0)
	{
		arg1 = SMEM_PC_OEM_AUD_LB_MIC1_TO_SPEAKER_START;
	}
	else if (temp == 1)
	{
		arg1 = SMEM_PC_OEM_AUD_LB_MIC1_TO_SPEAKER_STOP;
	}
	else if (temp == 2)
	{
		arg1 = SMEM_PC_OEM_AUD_LB_MIC2_TO_HEADSET_START;
	}
	else if (temp == 3)
	{
		arg1 = SMEM_PC_OEM_AUD_LB_MIC2_TO_HEADSET_STOP;
	}
	{
		loopback_mode = arg1;
		arg2 = 0;
		printk("%s, cust_mproc_comm1 %d %d\n",__func__, arg1, 0);    
		ret = cust_mproc_comm1(&arg1,&arg2);  
		if(ret)
		{
		  printk("%s, cust_mproc_comm1 fail\n",__func__);    
		}
	}
...
const struct file_operations debug_version_ops = {
	.open = debug_open,
	.write = debug_write_proc,
};

static void snd_debugfs_init(struct snd_ctxt *snd)
{
	struct dentry *dent;
	dent = debugfs_create_dir("msm_snd", 0);
	if (IS_ERR(dent))
		return;

	debugfs_create_file("operation", 0444, dent, snd, &debug_version_ops);
}
#else
static void snd_debugfs_init(struct snd_ctxt *snd)
{
...
static void snd_create_kernel_debuglevel(void)
{
	SND_PRINTK (0, "create snd kernel debuglevel!!!\n");

	if (kernel_debuglevel_dir!=NULL)
	{
		printk(KERN_ERR "kernel_debuglevel_dir snd_dll Done\n");
		debugfs_create_u32("snd_dll", S_IRUGO | S_IWUGO,
			kernel_debuglevel_dir, (u32 *)(&SND_DLL));
	}
	else
	{
		printk(KERN_ERR "kernel_debuglevel_dir snd_dll Fail\n");
	}
}
...
static int snd_probe(struct platform_device *pdev)
{
	...
	rc = device_create_file(snd_misc.this_device, &dev_attr_ec);
	if (rc) {
