...
static int __init board_adc_setup(char *options)
{
    if (!options || !*options)
        return 0;

    board_adc_value = simple_strtoul(options, NULL, 0);
    printk(KERN_INFO "%s: board_adc_value=%d\n", __func__, board_adc_value);

    return 0;
}
__setup("androidboot.board_adc=", board_adc_setup);

static ssize_t
socinfo_show_board_adc(struct sys_device *dev,
		     struct sysdev_attribute *attr,
		     char *buf)
{
	if (!socinfo) {
...
static ssize_t
socinfo_show_cust_sw_rev(struct sys_device *dev,
		     struct sysdev_attribute *attr,
		     char *buf)
{
	smem_vendor_id0_amss_data *amss_ptr;

	amss_ptr = smem_alloc (SMEM_ID_VENDOR0, sizeof(smem_vendor_id0_amss_data));

	return snprintf(buf, PAGE_SIZE, "%s", amss_ptr->cust_mob_sw_rev);
}

static ssize_t
socinfo_show_system_rev(struct sys_device *dev,
		     struct sysdev_attribute *attr,
		     char *buf)
{
...
static struct sysdev_attribute socinfo_qisda_files[] = {
	...
	_SYSDEV_ATTR(system_rev, 0444, socinfo_show_system_rev, NULL),
	_SYSDEV_ATTR(board_adc, 0444, socinfo_show_board_adc, NULL),
	_SYSDEV_ATTR(cust_sw_rev, 0444, socinfo_show_cust_sw_rev, NULL),
};
...
int __init socinfo_init(void)
{
	...
	printk("%s, cur_cpu %d, socinfo->v1.id %d\n", __func__, cur_cpu, socinfo->v1.id);
